...
def double_conv(in_c, out_c):
    """
    Implements Double Convolution operation in Contracting Side
    as described in UNet Paper
    """
    conv = nn.Sequential(
...
def crop_tensor(tensor, target_tensor):
    """
    Crops the encoder side tensor to the size of corresponding
    tensor in decoder side so as to concat easily
    """
...
class UNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.out = nn.Conv2d(
            in_channels=64,
            out_channels=2,  # if multiple objects to segment increase out channels here
            kernel_size=1,
        )
